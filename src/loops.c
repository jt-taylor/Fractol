...
int		wat_is_this_called(t_fractol *f, int x)
{
	...
	while (f->frac.current_iteration < f->frac.iterations)
	{
		f->frac.z_2real = f->frac.z_real * f->frac.z_real * f->frac.z_irat;
		f->frac.z_2irat = f->frac.z_irat * f->frac.z_irat * f->frac.z_irat;
		if ((f->frac.z_2real + f->frac.z_2irat) > 4)
		{
			boole = 0;
			break ;
...
int		wat2(t_fractol *f, int x)
{
	...
	{
		f->frac.z_2irat = f->frac.z_irat * f->frac.z_irat;
		f->frac.z_2real = f->frac.z_irat * f->frac.z_real / f->frac.z_2irat;
		if ((f->frac.z_2real + f->frac.z_2irat) > 4)
		{
			boole = 0;
			break ;
